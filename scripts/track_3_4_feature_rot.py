...
from object_pose_utils.utils.temporal_filtering_framework_feature import TempFilterEstimator
import torch
import numpy as np
...
def invalid_sample(data, video_len):
    ...
    for i in range(0, len(data)):
        obj, feat, quat = data[i]
        if len(obj) == 0:
            return True
    return False
                                                    
def evaluate_estimator(estimator, obj_id, interval, video_len):
    #feature_root = '/scratch/bokorn/results/dense_fusion_local_feat_test/'
    feature_root = '/scratch/bokorn/results/dense_fusion_local_feat_rot/'
    ...
    ycb_dataset = FeatureDataset(dataset_root = dataset_root,
                                 ...
                                 mode=mode, object_list = object_list)
...
for obj_id in obj_list:
    ...
    lik[obj_id] = lik_dict
#result_file.close()
np.savez("kalman_feature_4_3_rot.npz", err=err, lik=lik)
