...
def rotateImage(img, theta, meta_data):
    if(len(img.shape) == 2 or img.shape[2] == 1):
        interp = cv2.INTER_NEAREST
    else:
        interp = cv2.INTER_LINEAR
    
    if(meta_data is not None and 'camera_cx' in meta_data):
        center = (meta_data['camera_cx'], meta_data['camera_cy'])
    else:
    ...
    M = cv2.getRotationMatrix2D(center=center, angle=np.rad2deg(theta), scale=1.0)

    img_rot = cv2.warpAffine(img, M, dsize=(img.shape[1], img.shape[0]), flags = interp)
...
def rotateMetaData(meta_data, theta):
    ...
    meta_data_rot['mask'] = rotateImage(meta_data['mask'].astype(np.uint8), theta, meta_data) > .5
...
class InplaneRotator(object):
    ...
    def __call__(self, meta_data, img, depth, points, theta = None):
        ...
        if(img is not None):
            img = rotateImage(img, theta, meta_data)
        if(depth is not None):
            depth = rotateImage(depth, theta, meta_data)
