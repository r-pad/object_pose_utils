...
def accuracyAUC(error, max_theshold = 0.1):
    n = len(error)
    D = np.sort(error)
    D[D>max_theshold] = np.inf
    accuracy = np.cumsum(np.ones(n)) / n
    
    index = np.isfinite(D);
    D = D[index];
    accuracy = accuracy[index];

    mrec=np.concatenate([[0], D, [max_theshold]]);
    mpre=np.concatenate([[0], accuracy, accuracy[-1:]]);
    for i in range(1,len(mpre)):
        mpre[i] = max(mpre[i], mpre[i-1]);

    i = np.nonzero(mrec[1:] != mrec[:-1])[0] + 1
    ap = np.sum((mrec[i] - mrec[i-1]) * mpre[i]) / max_theshold;
    return ap
